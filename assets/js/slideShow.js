...
function slideShow(wrapperID, buttonID, startIconID, pauseIconID) {
  ...
  var globals = {
    ...
    wrapperID: wrapperID, // The ID of the <div> element that contains all of the <img> elements to be shown as a slide show.
    buttonID: buttonID, // The ID of the <button> element that toggles the slide show on and off.
	...
	startIconObject: null,
	pauseIconObject: null,
  ...
  function initializeGlobals() {   
    ...
    startIconObject = (document.getElementById(startIconID) ? document.getElementById(startIconID) : null);   
	pauseIconObject = (document.getElementById(pauseIconID) ? document.getElementById(pauseIconID) : null);   
  ...
  function initializeSlideShowMarkup() {  
    ...
    globals.wrapperObject.style.height = "100%"; */

	var maxSlideIndex = tallestSlideIndex();
	var tallestSlide = globals.slideImages[maxSlideIndex];
	globals.wrapperObject.innerHTML = tallestSlide.outerHTML + globals.wrapperObject.innerHTML;
	globals.slideImages = (globals.wrapperObject.querySelectorAll('img') ? globals.wrapperObject.querySelectorAll('img') : []);
    ...
    for (var i = 0; i < slideCount; i++) {
	  if (i==0) {
		...
		globals.slideImages[i].style.position = "relative";
	  } else if (i==1){
		globals.slideImages[i].style.opacity = 1; // Make the first slide visible.
		globals.slideImages[i].style.position = "absolute";
	  }else {
		globals.slideImages[i].style.opacity = 0;
		globals.slideImages[i].style.position = "absolute";          
	  }
	  globals.slideImages[i].style.top = 0;
	  globals.slideImages[i].style.left = 0;
    }
    globals.slideIndex = 1;
	
    if (globals.buttonObject) {
      //globals.buttonObject.textContent = globals.buttonStopText;
	  globals.buttonObject.innerHTML = pauseIconObject.outerHTML;
	  //globals.buttonObject.replaceChild(pauseIconObject, globals.buttonObject.childNodes[0]);
    }
  } // initializeSlideShowMarkup
  
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  
  function tallestSlideIndex() {
	  var maxRatio = 0;
	  var maxSlideIndex = 0;
	  var ratio;
	  var slideCount = globals.slideImages.length;
	  ...
	  for (var i=0; i < slideCount; i++) {
		  ratio = globals.slideImages[i].height / globals.slideImages[i].width;
		  if (ratio > maxRatio) {
			  maxRatio = ratio;
			  maxSlideIndex = i;
		  }
  ...
  function toggleSlideShow() {
    if (globals.slideShowRunning) {
      ...
      if (globals.buttonObject) { 
		...
		//globals.buttonObject.replaceChild(startIconObject, globals.buttonObject.childNodes[0]);
		globals.buttonObject.innerHTML = startIconObject.outerHTML;
      }
    }
    else {
      startSlideShow();
